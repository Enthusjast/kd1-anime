...
- 新增结构化 ScenePlan 字段、确定性计划编译器和批量计划审查；在写代码前检查
  时间线、可解析等式、多边形面积、画布边界和元素生命周期。
- 新增全片 `PlanningDraft`、`LessonSpec` 和 `TeachingGraph`；将数学断言、定义域、教学依赖、
  场景分配和时长约束固定在概要阶段，并在计划审查前执行确定性覆盖检查。
- 新增运行级 ElementManifest，记录元素身份、变量、依赖、语义状态、源定义和哈希，
  并只向相邻 Coder 注入所需的交接上下文。
- 新增 `StateLedger`，保存场景开闭状态、数学状态、代码/视频哈希和相邻场景边界证据。
- 新增正式渲染前的同 renderer Smoke Render、语义关键帧采样、即时视觉评估、
  status/logs/retry/cache 诊断命令和可选计划批准闸门。
- 新增默认启用的私有 SQLite LLM 响应缓存及调用事件统计。
- 新增使用独立 OpenAI-compatible 多模态端点的逐场景视觉质量门。
- 新增带时间戳和 SHA-256 的关键帧采样、严格结构化视觉报告与成片报告。
- 新增真实相邻场景边界关键帧评估；视觉数学问题回到 Planner，交接问题回到 Continuity，
  布局问题才进入 Coder 修复。
- 新增有界视觉修复、最佳候选恢复，以及可安全恢复的 manifest v3 视觉状态。
- `doctor --probe-visual-llm` 可验证独立模型是否实际接受图片消息。

### 改进

- 修复 CI 格式检查、恢复 dry-run 状态显示和恢复阶段 LLM 预检；清理陈旧运行前会核对并取消已知的 Slurm 作业。
- 安装器默认锁定 Manim Community Edition 0.20.1，并同步新增的计划审查与保守降级配置。
- `kd1-anime plan` 默认执行计划审查；连续性导出支持复合 Mobject helper，并固定跨场景变量名。
- RAG 索引记录源目录并检测源文件变更；渲染产物身份额外包含 Manim、FFmpeg 和 XeLaTeX 版本。
- 统一视频合并器与 Rich Live 仪表盘的输出行为，并让代码质量评估复用提交前确定性校验。
- 仪表盘新增“视觉”阶段；渲染完成后不再在视觉评估结束前提前标绿。
- 视觉端点故障记为 `unknown` 并保留成功渲染产物，不伪造评分。
- 合并前校验视觉收据与当前视频哈希，防止复用过期评估结果。
- 压缩连续性和代码审查上下文；严格结构化响应即使以 `finish_reason=length` 结束，
  也会先经过 JSON/Pydantic 校验，避免完整结果被误判为截断。
- dry-run 含失败场景时不再误标为完成；显式恢复可继续修复旧的失败 dry-run。
- Manifest 升级到 v5；v4 仅可只读查看，旧版清单不再猜测迁移，避免教学和连续性状态被静默误读。
- 辅助方法中的 `self.play/add/remove/clear` 会被生命周期检查阻断；计划错误不会再通过反复修改代码来掩盖。
...
### 新增
- **环境能力探针**：`doctor --probe` 可在不提交 Slurm 的情况下验证 FFmpeg、XeLaTeX `.xdv` 和当前 Manim renderer 的真实视频产物
- **批量中断恢复**：批量 Ctrl-C 会取消已登记的 Orchestrator/远端作业，并将任务与运行清单标记为可恢复的 interrupted 状态
...
- 收紧生成代码导入白名单：只允许精确的顶层纯计算模块，阻断模块路径、别名、通配符和 Manim 内部模块绕过
- 统一 Slurm 监控状态机，按当前作业目录竞态安全地发现嵌套 MP4；显式监控超时不再被旧配置覆盖
- Manifest、评估结果和报告使用带 fsync 的原子写入；视觉评估限制关键帧数量与单帧大小，并校验最终视频路径和哈希
