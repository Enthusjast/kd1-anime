...
def test_show_banner_does_not_scan_or_resume_historical_runs(monkeypatch):
    """启动交互会话时不扫描历史运行，恢复必须由用户显式执行。"""
    session = ChatSession()
    monkeypatch.setattr(
        session,
        "_check_interrupted_runs",
        lambda: (_ for _ in ()).throw(AssertionError("启动时不应读取历史运行")),
        raising=False,
    )
    session._show_banner()
