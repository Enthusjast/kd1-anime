...
## 技术可实现性
- 只描述可观察的视觉意图和数学状态，不预设某个 API 名称或动画类别。
- 方案必须能由当前 Manim 环境的标准 2D/3D 图形、公式、相机和属性动画组合实现；
  不确定的复杂效果应退化为可验证的基础图形、等式或标注。
- 同一个视觉意图可以有多种实现，交给 Technical Planner 和 Coder 根据 renderer、
  连续性合同和 Smoke Render 结果选择。
